...
image_dir = r"C:\Users\Mingming\Downloads\Multi_Label_dataset\Multi_Label_dataset\Images"
csv_file = r"C:\Users\Mingming\Downloads\Multi_Label_dataset\Multi_Label_dataset\train.csv"
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    zeros = torch.zeros(outputs.size())
                    preds = torch.where(torch.sigmoid(outputs)>0.5, ones , zeros)
            ...
            epoch_acc = running_corrects.double() / len(dataloaders[phase].dataset) / 25.0
...
class MovieDataset(Dataset):
    ...
    def __init__(self, root, label_file, transform=None, phase ='train'):
        #self.image_files = np.array([x.path for x in os.scandir(root) if  x.name.endswith(".jpg") or x.name.endswith(".png")])
        
        table = np.genfromtxt(label_file,dtype= str, delimiter=',', skip_header=1)
        rows = len(table)
        samples = int(0.04*rows)
        image_files = []

        if phase=='train':
            self.labels = np.array(table[0:samples, 2:], dtype = np.float32)
            for id in table[0:samples, 0]:
                image_files.append(os.path.join(root, str(id+".jpg"))) 
        else:
            self.labels = np.array(table[-samples+200:, 2:], dtype = np.float32)
            for id in table[-samples+200:, 0]:
                image_files.append(os.path.join(root, str(id+".jpg"))) 
...
image_datasets ={ x: MovieDataset(image_dir, csv_file, data_transforms[x], x) for x in ['train','val']}
...
scratch_criterion = nn.BCEWithLogitsLoss()
